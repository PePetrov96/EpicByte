...
class TextbookControllerTest {
    ...
    void setUp() {
        ...
        if (textbookRepository.count() == 0) {
            textbookRepository.save(returnTextbook());
        }
    ...
    @Test
    void viewTextbookProductDetails() throws Exception {
        mockMvc.perform(get("/textbook" + PRODUCT_DETAILS_URL + "/" + uuid)
                        .with(csrf()))
    ...
    @Test
    void displayTextbooksPage() throws Exception {
        mockMvc.perform(get(ALL_TEXTBOOKS_URL)
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(model().attributeExists("breadcrumbs"))
                .andExpect(model().attributeExists("productType"))
    ...
    void displayProductAddTextbookPage() throws Exception {
        mockMvc.perform(get(MODERATOR_PRODUCT_ADD_TEXTBOOK_URL)
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(model().attributeExists("linkType"))
                .andExpect(model().attributeExists("productType"))
    ...
    @Test
    @WithMockUser(roles = {"MODERATOR"})
    void handleProductAddTextbookPage() throws Exception {
        mockMvc.perform(post(MODERATOR_PRODUCT_ADD_TEXTBOOK_URL)
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(model().attributeExists("linkType"))
                .andExpect(model().attributeExists("productType"))
    ...
    @Test
    @WithMockUser(roles = {"MODERATOR"})
    void deleteTextbooks() throws Exception {
        mockMvc.perform(get(MODERATOR_TEXTBOOKS_DELETE_URL + uuid)
                        .with(csrf()))
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:" + ALL_TEXTBOOKS_URL));
    }
    ...
    private Textbook returnTextbook() {
